...
class SprintMetricsApp:
    ...
    def _generate_html_content(self, increment, current_sprint, sprints, metrics):
        ...
        try:
            start_sprint_str, end_sprint_str = sprint_range.split(' - ')
            start_sprint_val = float(start_sprint_str)
            end_sprint_val = float(end_sprint_str)
        except (ValueError, IndexError):
            # Fallback to current_sprint if range is not well-formed
            start_sprint_val = end_sprint_val = float(current_sprint)

        active_sprints = [s for s in sprints if start_sprint_val <= float(s) <= end_sprint_val]

        progress_html = ''
        for s in sprints:
            classes = ['progress-item']
            if s in active_sprints:
                classes.append("active")
                if s == active_sprints[0]:
                    classes.append("active-first")
                if s == active_sprints[-1]:
                    classes.append("active-last")

            class_str = ' class="' + ' '.join(classes) + '"'
            progress_html += f'<div{class_str}>{s}</div>'
...
<head>
    ...
    <style>
        * {{
        ...
        .progress-item.active.active-first {{
            border-top-left-radius: 20px;
            border-bottom-left-radius: 20px;
        }}

        .progress-item.active.active-last {{
            border-top-right-radius: 20px;
            border-bottom-right-radius: 20px;
        }}
        
        .progress-item.active.active-last::after {{
            content: '';
            position: absolute;
            right: -15px;
            top: 50%;
...
class TestSprintMetricsApp(unittest.TestCase):
    ...
    def test_html_generation_structure(self):
        ...
        self.app.increment.set(17)
        self.app.current_sprint.set("17.3")
        self.app.sprint_range.set("17.1 - 17.3") # Test a range
        ...
        # Verify active sprints are marked
        self.assertIn('class="progress-item active active-first">17.1</div>', html)
        self.assertIn('class="progress-item active">17.2</div>', html)
        self.assertIn('class="progress-item active active-last">17.3</div>', html)
        self.assertNotIn('class="progress-item active">17.4', html)
    ...
    def test_delete_history_item(self):
        ...
        self.app.history_file = Path("test_sprint_history.json")
    ...
    def test_corrupted_history_handling(self):
        ...
        self.app.history_file = Path("test_sprint_history.json")
        with open(self.app.history_file, 'w') as f:
